...
int main(int argc, char* argv[])
{
    ...
    if (argc!=4)
	{
		std::cout << "Usage: program message.txt key iv\n";
		exit(1);
	}
	...
	//ptext.copy(const_cast<char*>(lines.c_str()),lines.length(),0);
	secure_string ptext = const_cast<char*>(lines.c_str());
	//std::cout << "ptext=\n" << ptext << std::endl;

    byte key[KEY_SIZE], iv[BLOCK_SIZE];
    // gen_params(key, iv);
    // std::string skey = "0123456789012345678901234567890";
	std::string skey = argv[2];
	std::copy( skey.begin(), skey.end(), key);
	key[skey.length()] = 0;
	
	// std::string siv = "012345678901234";
	std::string siv = argv[3];
	...
	std::ofstream outfile("dmessage.txt");
	outfile << rtext;
	outfile.close();
